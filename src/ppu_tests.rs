...
use crate::ppu::{
    Ppu, DMG_PALETTE, DRAWING_CYCLES, HBLANK_CYCLES, MODE_DRAWING, MODE_HBLANK, MODE_OAM_SCAN,
    MODE_VBLANK, OAM_SCAN_CYCLES, SCANLINE_CYCLES,
};

// Helper to create a PPU instance, optionally with a specific GameBoyModel
fn setup_ppu(model_override: Option<GameBoyModel>) -> Ppu {
    // Changed SystemMode to GameBoyModel
    let model = model_override.unwrap_or(GameBoyModel::DMG); // Use GameBoyModel
    Ppu::new(model)
}

fn setup_ppu_dmg() -> Ppu {
    Ppu::new(GameBoyModel::DMG)
}
fn setup_ppu_cgb() -> Ppu {
    Ppu::new(GameBoyModel::CGBD)
} // Using CGBD as a default CGB model
...
fn compare_pixel(
    ...
    assert!(
        idx + 2 < fb.len(),
        "Framebuffer index out of bounds. Fb len: {}, idx+2: {}. At ({},{}): {}",
        fb.len(),
        idx + 2,
        x,
        y,
    ...
    assert_eq!(
        ...
        x, y, message, expected_rgb, pixel
    );
}
...
fn test_ppu_initialization_values() {
    ...
    assert_eq!(
        ppu_dmg.stat & 0b11,
        MODE_OAM_SCAN,
        "Initial STAT mode (ignoring LYC and bit 7)"
    );
    ...
    assert_eq!(
        ppu_dmg.model,
        GameBoyModel::DMG,
        "System mode should be DMG"
    ); // Changed system_mode to model
    if ppu_dmg.ly == ppu_dmg.lyc {
        assert_ne!(ppu_dmg.stat & (1 << 2), 0, "LYC=LY flag if ly==lyc");
    }
    ...
    assert_eq!(
        ppu_cgb.model,
        GameBoyModel::CGBD,
        "System mode should be CGBD"
    ); // Changed system_mode to model, and SystemMode::CGB to GameBoyModel::CGBD
    ...
    assert!(
        ppu_cgb.cgb_bg_palette_ram.iter().all(|&x| x == 0xFF),
        "Initial CGB BG Palette RAM"
    );
    assert!(
        ppu_cgb.cgb_obj_palette_ram.iter().all(|&x| x == 0xFF),
        "Initial CGB OBJ Palette RAM"
    );
}
...
fn test_lcdc_display_enable_master() {
    ...
    assert_eq!(
        ppu.stat & 0b11,
        MODE_HBLANK,
        "PPU mode should be HBLANK when LCD disabled"
    );
...
fn test_stat_mode_bits_when_lcd_disabled() {
    ...
    assert_eq!(
        stat_val & 0b11,
        0,
        "STAT mode bits should be 0 when LCD disabled"
    );
}
...
fn test_lcdc_bg_window_enable_dmg() {
    ...
    for i in 0..16 {
        ppu.vram[0][i] = 0xFF;
    } // Tile 0 is all color 3 (black with default BGP)
    ...
    compare_pixel(
        ...
        10,
        160,
        &DMG_PALETTE[bgp_col0_idx as usize],
        "BG is BGP color 0 if LCDC.0 off (DMG)",
    );
    ...
    compare_pixel(
        ...
        160,
        &DMG_PALETTE[bgp_col0_idx as usize],
        "BG still BGP color 0 with OBJ enabled",
    );
    compare_pixel(
        &ppu.framebuffer,
        0,
        ...
        160,
        &DMG_PALETTE[3],
        "Sprite renders if LCDC.0 off (DMG)",
    );
}
...
fn test_vram_oam_access_restrictions() {
    ...
    assert_eq!(
        ppu.read_byte(vram_addr),
        0xFF,
        "VRAM read restricted Mode 3"
    );
    ppu.write_byte(vram_addr, 0xAA);
    assert_ne!(ppu.vram[0][0], 0xAA, "VRAM write restricted Mode 3");

    ppu.stat = (ppu.stat & !0b11) | MODE_HBLANK;
    ppu.vram[0][0] = 0xBB;
    assert_eq!(ppu.read_byte(vram_addr), 0xBB, "VRAM read Mode 0");
    ppu.write_byte(vram_addr, 0xCC);
    assert_eq!(ppu.vram[0][0], 0xCC, "VRAM write Mode 0");

    ppu.stat = (ppu.stat & !0b11) | MODE_OAM_SCAN;
    assert_eq!(ppu.read_byte(oam_addr), 0xFF, "OAM read restricted Mode 2");
    ppu.write_byte(oam_addr, 0xAA);
    assert_ne!(ppu.oam[0], 0xAA, "OAM write restricted Mode 2");

    ppu.stat = (ppu.stat & !0b11) | MODE_VBLANK;
    ppu.oam[0] = 0xCC;
    assert_eq!(ppu.read_byte(oam_addr), 0xCC, "OAM read Mode 1");
    ppu.write_byte(oam_addr, 0xDD);
    assert_eq!(ppu.oam[0], 0xDD, "OAM write Mode 1");
}
...
fn test_scanline_cycle_modes() {
    ...
    ppu.ly = 0;
    ppu.cycles = 0;
    ppu.set_mode(MODE_OAM_SCAN); // Start of scanline
    assert_eq!(ppu.stat & 0b11, MODE_OAM_SCAN);
    ppu.tick(OAM_SCAN_CYCLES);
    assert_eq!(ppu.stat & 0b11, MODE_DRAWING);
    ppu.tick(DRAWING_CYCLES);
    assert_eq!(ppu.stat & 0b11, MODE_HBLANK);
    assert_eq!(ppu.ly, 0);
...
fn test_lyc_ly_interrupt() {
    ...
    for i in 0..4 {
        ...
        assert!(
            (ppu.stat & (1 << 2)) == 0,
            "LYC flag should not be set for LY={}",
            i
        );
        if let Some(InterruptType::LcdStat) = interrupt {
            panic!("Unexpected STAT int for LY={}", i);
        }
    }

    ppu.ly = 5;
    ppu.cycles = 0; // LY becomes 5
    ...
    assert_ne!(
        ppu.stat & (1 << 2),
        0,
        "LYC flag should be set for LY=5, LYC=5"
    );
    ...
    ppu.ly = 4;
    ppu.cycles = 0;
    ppu.tick(SCANLINE_CYCLES); // LY becomes 5
    ...
    ppu.ly = 5;
    ppu.lyc = 5;
    ppu.stat |= 1 << 6; // Enable LYC interrupt
    assert_eq!(
        ppu.check_lyc_ly(),
        Some(InterruptType::LcdStat),
        "check_lyc_ly should signal STAT interrupt"
    );
    ...
    assert_eq!(
        ppu.check_lyc_ly(),
        None,
        "check_lyc_ly should not signal if LY != LYC"
    );
...
fn test_sprite_flips_dmg() {
    ...
    ppu.vram[0][0] = 0x80;
    ppu.vram[0][1] = 0x00; // Row 0
    for i in 2..14 {
        ppu.vram[0][i] = 0x00;
    } // Middle rows blank
    ppu.vram[0][14] = 0x01;
    ppu.vram[0][15] = 0x00; // Row 7
    ...
    compare_pixel(
        ...
        160,
        &DMG_PALETTE[1],
        "X-flip, last pixel becomes first",
    );
    ...
    compare_pixel(
        ...
        160,
        &DMG_PALETTE[1],
        "Y-flip, row 0 uses tile data from row 7, last pixel",
    );
    ...
    compare_pixel(
        ...
        160,
        &DMG_PALETTE[1],
        "X & Y-flip, row 0 uses tile data from row 7, first pixel of that row",
    );
}
...
fn test_cgb_palette_io_access() {
    ...
    assert_eq!(
        ppu.bcps, 0x05,
        "BCPS should not auto-increment if bit 7 is not set"
    );
    ...
    assert_eq!(
        ppu.bcps,
        0x80 | ((5 + 1) & 0x3F),
        "BCPS should auto-increment"
    );
    assert_eq!(
        ppu.read_byte(0xFF69),
        ppu.cgb_bg_palette_ram[6],
        "Reading BCPD after increment"
    );
    ...
    assert_eq!(
        ppu.ocps,
        0x80 | ((0xA + 1) & 0x3F),
        "OCPS should auto-increment"
    );
    assert_eq!(
        ppu.read_byte(0xFF6B),
        ppu.cgb_obj_palette_ram[0xB],
        "Reading OCPD after increment"
    );
}
