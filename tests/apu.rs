...
fn build_channel_1_delay_rom() -> Vec<u8> {
    fn subtest(nops: u8, freq: u8, store: u16, rom: &mut Vec<u8>) {
        rom.extend([
            0xAF, // XOR A
            0xE0, 0x26, // LDH [NR52],A
            0x2F, // CPL
    ...
    // First pass to compute store address
    let mut tmp = Vec::new();
    for nops in 0u8..=15 {
        subtest(nops, 0xFF, 0, &mut tmp);
    }
    for nops in 0u8..=15 {
        subtest(nops, 0xFE, 0, &mut tmp);
    }
    let store = (rom.len() + tmp.len()) as u16;

    for nops in 0u8..=15 {
        subtest(nops, 0xFF, store, &mut rom);
    }
    for nops in 0u8..=15 {
        subtest(nops, 0xFE, store, &mut rom);
    }
