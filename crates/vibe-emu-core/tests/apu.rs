...
#[test]
fn nr52_power_cycle_preserves_output_queue_and_sample_clock() {
    for model in [Model::default(), Model::Cgb(CgbRevision::RevE)] {
        for rate in [44_100, 48_000] {
            let mut apu = Apu::new(model);
            let consumer = apu.enable_output(rate);
            let mut div = 0;
            tick_machine(&mut apu, &mut div, 40);
            assert_eq!(consumer.pop_stereo(), None);

            apu.write_reg(0xFF26, 0);
            tick_machine(&mut apu, &mut div, 60);
            assert_eq!(
                consumer.pop_stereo(),
                Some((0, 0)),
                "NR52 must preserve the host queue and sample clock: {model:?}, {rate} Hz"
            );
            ...
            for _ in 0..3 {
                ...
                apu.write_reg(0xFF17, 0x08); // DAC on, digital input remains zero
                tick_machine(&mut apu, &mut div, 100);
                let (left, right) = consumer
                    .pop_stereo()
                    .expect("audio resumes on the same queue");
                assert!(left > 0);
                assert_eq!(left, right);
                ...
                apu.write_reg(0xFF26, 0);
                tick_machine(&mut apu, &mut div, 100);
                assert_eq!(consumer.pop_stereo(), Some((0, 0)));
                while let Some(sample) = consumer.pop_stereo() {
...
#[test]
fn disabled_dac_is_silent_even_when_another_dac_is_on() {
    let dacs = [
        (0xFF12, 0x08),
        (0xFF17, 0x08),
        (0xFF1A, 0x80),
    ...
    for model in [Model::default(), Model::Cgb(CgbRevision::RevE)] {
        for (channel, &(register, value)) in dacs.iter().enumerate() {
            for pan in [0x10, 0x01, 0x11] {
                let mut apu = Apu::new(model);
                apu.write_reg(0xFF26, 0);
                apu.write_reg(0xFF26, 0x80);
                let consumer = apu.enable_output(48_000);
                // Keep the HPF connected using a different, unrouted DAC.
                let (other_register, other_value) = dacs[(channel + 1) % dacs.len()];
                apu.write_reg(other_register, other_value);
                apu.write_reg(0xFF24, 0x77);
                apu.write_reg(0xFF25, pan << channel);
                let mut div = 0;
                tick_machine(&mut apu, &mut div, 88);
                assert_eq!(
                    consumer.pop_stereo(),
                    Some((0, 0)),
                    "disabled DAC {channel} must not add a DC offset: {model:?}"
                );

                // A DAC converts digital zero to a positive analog level even
                // without triggering its channel. NR52/PCM stay inactive/zero.
                apu.write_reg(register, value);
                tick_machine(&mut apu, &mut div, 88);
                let on = consumer.pop_stereo().unwrap();
                assert_eq!(on.0 > 0, pan & 0x10 != 0);
                assert_eq!(on.1 > 0, pan & 0x01 != 0);
                assert_eq!(apu.read_reg(0xFF26) & 0x0F, 0);
                assert_eq!(apu.pcm_samples(), [0; 4]);

                // Removing that offset creates the opposite edge through the
                // HPF; treating DAC-off as digital zero would lose this edge.
                apu.write_reg(register, 0);
                tick_machine(&mut apu, &mut div, 88);
                let off = consumer.pop_stereo().unwrap();
                assert_eq!(off.0 < 0, pan & 0x10 != 0);
                assert_eq!(off.1 < 0, pan & 0x01 != 0);
            }
        }
    }
}
...
#[test]
fn nr52_power_cycle_preserves_frontend_audio_settings() {
    let mut apu = Apu::new(Model::default());
    let consumer = apu.enable_output(48_000);
    apu.set_speed(2.0);
    apu.write_reg(0xFF26, 0);
    ...
    let mut div = 0;
    tick_machine(&mut apu, &mut div, 100);
    assert_eq!(consumer.pop_stereo(), None, "fast-forward remains muted");
    apu.set_speed(1.0);
    tick_machine(&mut apu, &mut div, 100);
    assert_eq!(consumer.pop_stereo(), Some((0, 0)));
